...
class Dataset(object):
    ...
    def build_iterator(self, pair_gen : PairGenerator):
        ...
        return Inputs(element[self.img1_resized], element[self.img2_resized],
                      element[PairGenerator.label])


    def _read_image_and_resize(self, pair_element):
        target_size = [128, 128]
        img1_file = tf.io.read_file(pair_element[PairGenerator.person1])
        img2_file = tf.io.read_file(pair_element[PairGenerator.person2])
        
        img1 = tf.io.decode_image(img1_file)
        img2 = tf.io.decode_image(img2_file)
        
        #Let tensorflow know that the loaded images have unknown dimensions, and 3 color channels
        img1.set_shape([None, None, 3])
        img2.set_shape([None, None, 3])
        
        #resize to model input size
        img1_resized = tf.image.resize(img1, target_size)
        img2_resized = tf.image.resize(img2, target_size)
